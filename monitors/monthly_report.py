...
async def fetch_recently_completed_count(jira_email, jira_token, jira_project_key, jira_url, days: int = 7) -> int:
    """Количество задач проекта, переведенных в статус категории "Done" за
    последние `days` дней (по полю resolutiondate) — для карточки-виджета
    "N задач выполнено за последние N дней" в Mini App (аналог гаджета
    "Number chart" на дашборде Jira)."""
    base_url = jira_url.rstrip("/")
    auth = aiohttp.BasicAuth(jira_email, jira_token)
    headers = {"Accept": "application/json", "Content-Type": "application/json"}

    jql = f'project="{jira_project_key}" AND statusCategory = Done AND resolutiondate >= -{days}d'
    search_url = f"{base_url}/rest/api/3/search/jql"
    total = 0
    next_page_token = None
    ...
    try:
        async with aiohttp.ClientSession(auth=auth, headers=headers) as session:
            while True:
                ...
                async with session.post(search_url, json=payload) as resp:
                    if resp.status != 200:
                        logger.error(f"Ошибка подсчета выполненных задач: {resp.status}")
                        return total
                    data = await resp.json()
        ...
        logger.error(f"Ошибка запроса выполненных задач: {e}")
        return total
...
async def fetch_due_soon_count(jira_email, jira_token, jira_project_key, jira_url, days: int = 7) -> int:
    """Количество задач проекта, у которых срок выполнения (due date) — от
    сегодня и в ближайшие `days` дней — для карточки-виджета "N задач срок
    выполнения — в ближайшие N дней" в Mini App (аналог гаджета "Number
    chart" на дашборде Jira, фильтр "Дата выполнения задачи" From: Today,
    To: Next N days)."""
    base_url = jira_url.rstrip("/")
    auth = aiohttp.BasicAuth(jira_email, jira_token)
    headers = {"Accept": "application/json", "Content-Type": "application/json"}
    ...
    jql = (
        f'project="{jira_project_key}" AND due >= startOfDay() '
        f'AND due <= startOfDay("+{days}d")'
    )
    search_url = f"{base_url}/rest/api/3/search/jql"
    total = 0
    next_page_token = None
    ...
    try:
        async with aiohttp.ClientSession(auth=auth, headers=headers) as session:
            while True:
                ...
                async with session.post(search_url, json=payload) as resp:
                    if resp.status != 200:
                        logger.error(f"Ошибка подсчета задач по сроку выполнения: {resp.status}")
                        return total
                    data = await resp.json()
        ...
        logger.error(f"Ошибка запроса задач по сроку выполнения: {e}")
        return total
